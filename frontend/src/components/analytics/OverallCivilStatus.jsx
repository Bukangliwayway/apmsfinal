...
const OverallCivilStatus = () => {
  const { data: overallCivilStatus, isLoading: isLoadingCivilStatus } =
    useOverallCivilStatus();

  if (isLoadingCivilStatus) {
    return (
      <Box>
  ...
  const maxItem = overallCivilStatus?.data?.responses.reduce(
    (max, current) => (current.value > max.value ? current : max),
    { value: -Infinity }
  );

  const sumOfValues = overallCivilStatus?.data?.responses.reduce(
  ...
  return (
    <ResponsivePie
      data={overallCivilStatus?.data?.responses}
...
export default OverallCivilStatus;
