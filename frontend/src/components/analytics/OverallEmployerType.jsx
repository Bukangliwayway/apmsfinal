...
const OverallEmployerType = () => {
  ...
  const maxItem = overallEmployerType?.data?.responses.reduce(
    (max, current) => (current.value > max.value ? current : max),
    { value: -Infinity }
  );

  const sumOfValues = overallEmployerType?.data?.responses.reduce(
  ...
  return (
    <ResponsivePie
      data={overallEmployerType?.data?.responses}
...
export default OverallEmployerType;
